...
7. **Present.** Produce:
   ...
   - An interactive dependency graph via `ccpm-scheduler graph schedule.csv project-network.html --tasks tasks.csv --title "..."` — a standalone HTML file (vis-network from a CDN, data embedded; no server or build step) the user opens in a browser to zoom, pan, drag nodes, and click a task to inspect its schedule, resources, and links. The Gantt shows *when*; this shows *why* — the network structure and merge points that are hard to read off bars. Always pass `--tasks`: it adds each task's realistic estimate next to its scheduled optimal duration (tooltip and inspector), which is what the team debates when reviewing whether the optimal/realistic balance is right
