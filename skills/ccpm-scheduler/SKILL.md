...
The CLI comes from the [ccpm-scheduler](https://github.com/rnwolf/ccpm-scheduler) Python package. Run it with uv as `uvx --from git+https://github.com/rnwolf/ccpm-scheduler ccpm-scheduler <subcommand> ...` — written below simply as `ccpm-scheduler ...`. If the `ccpm-scheduler` command is already on PATH (installed via `uv tool install` or pip), use it directly; without uv, `pip install git+https://github.com/rnwolf/ccpm-scheduler` provides it. Exit codes are a contract: 0 = ok, 1 = problems found (report still printed), 2 = bad invocation; add `--json` to any subcommand for machine-readable output, and accept input either as the CSV files below or as one JSON document (`ccpm-scheduler schema network` describes it).
